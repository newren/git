...
static void write_pack_file(void)
{
	...
	do {
		...
		if (!pack_to_stdout) {
			...
			if (mark_bad_deltas) {
				struct strbuf marker_tmp = STRBUF_INIT;
				size_t tmpname_len = tmpname.len;
				int fd;

				fd = odb_mkstemp(the_repository->objects,
						 &marker_tmp,
						 "pack/tmp_baddeltas_XXXXXX");
				if (close(fd))
					die_errno(_("unable to close '%s'"),
						  marker_tmp.buf);

				strbuf_addstr(&tmpname, "baddeltas");
				if (finalize_object_file(the_repository,
							 marker_tmp.buf,
							 tmpname.buf))
					die(_("unable to rename temporary file to '%s'"),
					    tmpname.buf);
				strbuf_release(&marker_tmp);
